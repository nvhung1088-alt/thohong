...
- [x] Sửa `server.js`
  ...
  - [x] Tối ưu repo `nvhung1088-alt/thohong`
  - [x] Thêm aria-label cho input số lượng (`qty-${p.id}`) trong danh sách sản phẩm
  - [x] Thêm aria-label cho input số lượng (`qty-modal-${p.id}`) trong modal chi tiết
  - [x] Thêm aria-label cho link nổi `#fcZalo` và `#fcHotline`
  - [x] Khắc phục CLS: Pre-render banner tĩnh trực tiếp trong HTML
  - [x] Commit & Push lên origin main
